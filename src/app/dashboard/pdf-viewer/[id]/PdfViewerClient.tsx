  ...
  return (
    ...
    >
      ...
      <header className="h-14 sm:h-16 px-3 sm:px-6 bg-[#040e1b]/95 backdrop-blur-xl flex items-center justify-between gap-2 sm:gap-4 z-30 shrink-0 shadow-lg shadow-black/40">
        ...
        {/* Right: Security & Student Info / Login */}
        <div className="flex items-center gap-2.5 shrink-0">
          {student ? (
            ...
            <Link
              href={`/login?redirect=${encodeURIComponent(`/dashboard/pdf-viewer/${material.id}`)}`}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-950/90 hover:bg-cyan-900 border border-cyan-500/50 hover:border-cyan-400 text-cyan-300 hover:text-white text-xs font-mono font-bold transition shadow-sm"
            >
