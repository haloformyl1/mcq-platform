...
export default function GoldUpgradeCelebrationModal({ student }: CelebrationModalProps) {
  ...
  useEffect(() => {
    if (!student || student.subscriptionStatus !== "PAID") {
      return;
    }
    ...
    const expiryKey = student.subscriptionExpiresAt 
      ...
      : "active";
    const storageKey = "piechem_gold_celebrated_" + student.id + "_" + expiryKey;
  ...
  useEffect(() => {
    ...
    const colors = ["#f59e0b", "#fbbf24", "#fef08a", "#38bdf8", "#10b981", "#ec4899", "#8b5cf6"];
    ...
    for (let i = 0; i < 110; i++) {
      particles.push({
        x: canvas.width * 0.5 + (Math.random() - 0.5) * 320,
        y: canvas.height * 0.35 + (Math.random() - 0.5) * 120,
        size: Math.random() * 8 + 4,
        color: colors[Math.floor(Math.random() * colors.length)],
        speedX: (Math.random() - 0.5) * 12,
        speedY: Math.random() * -14 - 3,
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-in fade-in duration-300">
      ...
      <div 
        ...
        className="relative z-20 w-full max-w-lg rounded-3xl bg-gradient-to-b from-[#0f1d32] via-[#091322] to-[#040812] border-2 border-amber-500/40 p-6 md:p-8 text-center shadow-[0_0_80px_rgba(245,158,11,0.25)] animate-in zoom-in-95 duration-300"
      >
        ...
        <p className="text-sm text-slate-300 mb-5 leading-relaxed">
          Congratulations <strong className="text-white">{student?.name || "Student"}</strong>! Administrator <strong className="text-amber-300">Arghyadeep Roy</strong> has approved your transaction. Your 30-Day All-Access Pass is now active.
        </p>

        <div className="bg-white/5 border border-amber-500/20 rounded-xl p-3 mb-5 flex items-center justify-between text-left">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[11px] font-bold text-amber-400 uppercase tracking-wider">Pass Validity</div>
              <div className="text-xs text-white font-medium">Valid until {validUntilStr}</div>
            </div>
          </div>
        ...
        <button
          onClick={handleClose}
          className="w-full py-3.5 px-6 rounded-xl font-extrabold text-sm text-black bg-gradient-to-r from-amber-400 via-amber-500 to-yellow-400 hover:from-amber-300 hover:to-yellow-300 shadow-[0_0_25px_rgba(245,158,11,0.4)] hover:scale-[1.02] active:scale-[0.98] transition-all"
        >
          🚀 Start Practicing Tests Now
        </button>

        <p className="text-[11px] text-slate-500 mt-3">
          Need help with your subscription? Call or WhatsApp Administrator: <a href="tel:9830507435" className="text-amber-400 hover:underline">9830507435</a>
        </p>
      </div>
    </div>
  );
