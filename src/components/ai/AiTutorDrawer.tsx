...
export default function AiTutorDrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 w-full h-full flex flex-col bg-[#050102] text-slate-100 overflow-hidden select-text animate-in fade-in duration-200 font-sans">
      {/* Atmospheric Radial Burgundy Glow */}
      <div
        className="pointer-events-none absolute inset-0 opacity-60"
        style={{
          background: "radial-gradient(ellipse 80% 50% at 50% 15%, rgba(159, 18, 57, 0.12) 0%, rgba(88, 28, 28, 0.04) 45%, transparent 75%)"
        }}
      />

      {/* Subtle STEM Orbital Lattice Background (~3% opacity) */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden opacity-[0.035]">
        <svg className="w-full h-full" xmlns="http://www.w3.org/2000/svg">
          ...
          <ellipse cx="50%" cy="30%" rx="380" ry="120" fill="none" stroke="#f43f5e" strokeWidth="1" strokeDasharray="4 8" />
          <ellipse cx="50%" cy="30%" rx="520" ry="160" fill="none" stroke="#f43f5e" strokeWidth="1" strokeDasharray="3 6" opacity="0.6" />
        </svg>
      </div>

      {/* Top Header - Fixed & Pinned at the Very Top */}
      <header className="relative z-30 shrink-0 h-16 border-b border-red-950/60 px-4 sm:px-8 flex items-center justify-between bg-[#080204]/90 backdrop-blur-2xl">
        <div className="flex items-center gap-3">
          {/* AI Brand Icon: Translucent dark red with crimson border, subtle radial glow & gentle pulse */}
          <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-b from-red-600/20 via-rose-950/30 to-black/80 border border-red-500/40 shadow-[0_0_15px_rgba(239,68,68,0.2)]">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <h2 className="text-base sm:text-lg font-black tracking-tight text-white flex items-center gap-1.5 font-sans">
                PIECHEM
                <span className="bg-gradient-to-r from-red-500 via-rose-400 to-red-300 bg-clip-text text-transparent">
                  AI
                </span>
              </h2>
              <span className="px-2 py-0.5 rounded-md bg-red-950/90 border border-red-500/30 text-[9.5px] font-extrabold uppercase tracking-wider text-rose-300">
                STEM Tutor
              </span>
              ...
              {quota && (
                quota.isUnlimited ? (
                  <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-500/15 border border-amber-500/30 text-[10px] font-bold uppercase tracking-wider text-amber-300 shadow-sm">
                    <Sparkles className="w-3 h-3 text-amber-400" />
                    Gold Unlimited
                  </span>
                ) : (
                  <span className={`hidden sm:inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm border ${
                    quota.remaining === 0
                      ? 'bg-rose-950/80 border-rose-500/50 text-rose-300'
                      : 'bg-red-950/60 border-red-500/30 text-rose-200'
                  }`}>
      ...
      >
        <div className={`max-w-4xl lg:max-w-5xl mx-auto w-full flex flex-col ${messages.length === 0 ? 'h-full justify-center' : 'space-y-5'}`}>
          ...
          {messages.length === 0 && (
            ...
            >
              <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-b from-red-600/20 via-rose-950/30 to-black/90 border border-red-500/40 shadow-[0_0_25px_rgba(239,68,68,0.25)] sm:h-16 sm:w-16">
              ...
              <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight text-white font-sans">
                {language === 'bn' ? (
                  ...
                  <>
                    Your Personal{" "}
                    <span className="bg-gradient-to-r from-white via-rose-100 to-red-400 bg-clip-text text-transparent">
                      STEM Tutor
                    </span>
                  </>
                )}
                ...
                {PROMPT_STARTERS.map((card, idx) => {
                  ...
                  return (
                    <button
                      ...
                      className="group relative p-3 sm:p-3.5 rounded-xl bg-[#0c0305]/85 hover:bg-[#140608] border border-red-950/80 hover:border-red-500/40 shadow-sm hover:shadow-[0_4px_20px_rgba(220,38,38,0.12)] hover:-translate-y-0.5 transition-all duration-200 cursor-pointer text-left"
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <span className={`text-[9px] uppercase font-extrabold tracking-wider px-2 py-0.5 rounded-md border ${(card as any).badgeColor || 'bg-red-950/60 text-rose-300 border-red-500/30'}`}>
                          {card.subject}
                        </span>
                        <div className="flex items-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity text-slate-300">
                          <Icon className="h-3.5 w-3.5" />
                          <ArrowRight className="h-3 w-3 -translate-x-0.5 group-hover:translate-x-0.5 transition-transform text-rose-400" />
                        </div>
                      </div>
                      <h4 className="text-xs sm:text-sm font-bold text-white group-hover:text-rose-100 transition-colors">
                        {card.title}
                      </h4>
      ...
      <footer className="relative z-30 shrink-0 border-t border-red-950/80 bg-[#090203]/95 px-3 sm:px-8 py-2 sm:py-3 backdrop-blur-xl">
        <div className="max-w-4xl lg:max-w-5xl mx-auto w-full space-y-2">
          ...
          <form
            ...
            className="rounded-2xl border border-red-950/80 bg-[#0d0305]/95 hover:border-red-900/60 focus-within:border-red-500/60 focus-within:shadow-[0_0_20px_rgba(239,68,68,0.2)] p-2 sm:p-2.5 transition-all shadow-xl"
          >
            <textarea
              ref={inputRef}
              value={input}
              ...
              className="w-full resize-none bg-transparent px-3 py-1 text-sm text-white placeholder-slate-500 border-0 outline-none focus:outline-none focus:ring-0 focus:border-0 leading-relaxed no-scrollbar"
